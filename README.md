...
(*NOTE:* `vickitrix` versions < 0.1.2 didn't work with the default rules because evidently, the Twitter Streaming API is sensitive ti case in handles. (I'd written `vickicryptobot` when I should have written `Vickicryptobot` in [`vickitrix/rules/vicki.py`](vickitrix/rules/vicki.py).) So make sure you get the capitalization of any handles you specify in your rules right! And if you've already followed the instructions below, you can upgrade to the latest version of `vickitrix` with
